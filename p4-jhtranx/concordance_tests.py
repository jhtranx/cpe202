...
class TestList(unittest.TestCase):
    ...
    def test_00_emptyfiles(self):
        conc = Concordance()
        with self.assertRaises(FileNotFoundError):
            conc.load_stop_table("notreal.txt")

        conc = Concordance()
        with self.assertRaises(FileNotFoundError):
            conc.load_concordance_table("notreal.txt")
    ...
    def test_DRAINED_OUTTTTT_BABY(self):
        conc = Concordance()
        conc.load_stop_table("stop_words.txt")
        conc.load_concordance_table("ecco2k.txt")
        conc.write_concordance("ecco2k_con.txt")
        err = subprocess.call("diff -wb ecco2k_con.txt ecco2k_sol.txt", shell = True)
        self.assertEqual(err, 0)
